...
class QuakeLauncher:
    ...
    def on_mod_select(self, event):
        ...
        self.preview_title.config(text=f"Mod: {m_name}")
        # title = self.get_map_title(mod, map_n)
    ...
    # (Including them for completeness in your local file is fine)
    ...
    def update_map_stats_display(self, mod_path, map_name):
        ...
        if entity_text:
            ...
            #self.map_info_label.config(text=f"Monsters ({current_skill}): {m} | Secrets: {s}")
